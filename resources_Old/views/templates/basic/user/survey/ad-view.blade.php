    ...
    @if($demo_route)
        <div class="survey-area mt-30">
            ...
            <div class="panel-card-body">
                <div class="row justify-content-center mb-30-none">
                    <div class="col-xl-12 col-md-12 col-sm-12 mb-30">
                        <p>{{__('You will not getting paid for this because you already watch this ad.')}}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="panel-card-body">
            <div class="row justify-content-center mb-30-none">
                <div class="video-container" id="video-container">
                    ...
                    <video controls class="video" id="video" preload="metadata" style="width: 100%">
                        <source src="{{$survey->video_url}}" type="video/mp4"></source>
                    </video>

                    <div class="video-controls hidden" id="video-controls">
                        <div class="video-progress">
                            <progress class="hidden" id="progress-bar" value="0" min="0"></progress>
                            <input class="seek hidden" id="seek" value="0" min="0" type="range" step="1">
                        ...
                        <div class="bottom-controls">
                            <div class="left-controls">
                                ...
                                <div class="volume-controls">
                                    <button data-title="Mute (m)" class="volume-button hidden" id="volume-button">
                                        <svg>
                                    ...
                                    <input class="volume hidden" id="volume" value="1"
                ...
                <div class="col-md-6 col-sm-8 mb-30">
                    <div class="survey-list-item">
                        <div class="survey-list-body">
                            <div class="survey-list-thumb">
                                ...
                                <!--</video>-->
{{--                                <video src="{{$survey->video_url}}" controls style="width: 100%"></video>--}}
...
{{--                                    <h3 class="title">{{__($survey->category->name)}}</h3>--}}
{{--                                    --}}{{--                                    <div class="survey-price">@lang('Reward'): {{$item->questions->count() * $general->paid_amount}} {{__($general->cur_text)}}</div>--}}
{{--                                </div>--}}
{{--                                <p>{{__($survey->p_name)}}</p>--}}
{{--                                <p>Price: {{$survey->p_mrp - $survey->discount}} INR</p>--}}
{{--                                <p>Views Left: {{$res_views}}</p>--}}
{{--                            </div>--}}
                        </div>
                        <div class="survey-list-footer bg--primary text-center mt-30 d-none" id="srv-btn">
                            <div class="survey-btn">
                                <a href="#" class="text-white text-center" id="question-url">@lang('Start Questionaire')</a>
                            </div>
                        </div>
                        @if(!is_null($survey->purchas_url))
                            <div class="survey-list-footer text-center mt-30">
                                Purchase Link: <a href="{{$survey->purchas_url}}" target="_blank">{{$survey->purchas_url}}</a>
                            </div>
                        @endif
{{--                        @foreach($survey->questions as $sq)--}}
{{--                            <div class="survey-list-footer text-center mt-30">--}}
{{--                                <div class="font-weight-bold">Questions {{$loop->iteration}}</div>--}}
{{--                                <ul>--}}
{{--                                    @if(!is_null($sq->custom_question))--}}
{{--                                        <li>{{$sq->custom_question}}</li>--}}
{{--                                    @else--}}
{{--                                        <li>{{$sq->question}}</li>--}}
{{--                                    @endif--}}
{{--                                </ul>--}}
...
{{--                        @endforeach--}}
    ...
    <div id="curr-timestamp" data-id="{{Carbon\Carbon::now()->timestamp}}"></div>
    ...
    @push('script')
        <script>
            ...
            function updateTimeElapsed() {
                ...
                const t = formatTime(videoDuration - time);
                // console.log(t)
                if(t.minutes == '00' && t.seconds == '00') {
                    // console.log('video end');
                    $('#srv-btn').removeClass('d-none');
                    let tstamp = $('#curr-timestamp').data('id');
                    let srv_id_r = $('#srv-id-r').data('id');
                    localStorage.removeItem('time');
                    localStorage.setItem('time', tstamp);
                    $('#question-url').attr('href', `/user/survey/${srv_id_r}/qustions?t=${tstamp}`);
                }
                timeElapsed.innerText = `${t.minutes}:${t.seconds}`;
                timeElapsed.setAttribute('datetime', `${t.minutes}m ${t.seconds}s`);
            }
