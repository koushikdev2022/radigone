                                ...
                                <div class="card mb-30 border--primary">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <a href="javascript:void(0)" data-id="{{$data->id}}" data-resource="{{$data}}"
                                           ...
                                           data-toggle="modal" data-target="#exampleModal">@lang('Buy Now')</a>
                                    </div>
                                </div><!-- card end -->
                            </div>
                        @endforeach
    ...
    <div class="modal fade" id="exampleModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('surveyor.buy_views.deposit.insert')}}" method="post">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="cat_price" id="cat_price" value="{{$category->price}}">
                        </div>
                        <div class="form-group">
                            <label>@lang('Number of Views'):</label>
                            <input id="add_views" type="text" class="form-control form-control-lg" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')" name="add_views" placeholder="0.00" required=""  value="{{old('add_views')}}">
                        </div>
                        <div class="form-group">
                            <label>@lang('Payable Amount'):</label>
                            <div class="input-group">
                                <input id="amount" type="text" class="form-control form-control-lg" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')" name="amount" placeholder="0.00" required=""  value="{{old('amount')}}" readonly>
...
@push('script')
    <script>
        ...
        $('#add_views').on('keyup', function () {
            let price = $('#cat_price').val();
            let views = $(this).val();
            let amount = price * views;
            $('#amount').val(amount);
