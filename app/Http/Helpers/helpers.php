...
function send_email($user, $type = null, $shortCodes = [])
{
    ...
    $dt['subject'] = $email_template->subj;
    $dt['content'] = $message;

    return Mail::to($user->email)->send(new \App\Mail\MainMail($dt));
...
function send_php_mail($receiver_email, $receiver_name, $sender_email, $subject, $message)
{
    ...
    $headers = "From: admin@radigone.com  \r\n";
    $headers .= "Reply-To: admin@radigone.com \r\n";
...
//    ];
    ...
    try {
        Mail::to($email)->send(new \App\Mail\MainMail($dt));
...
{
    function imageBase64($image_path)
    {
...
if(!function_exists('surveySession'))
{
        function surveySession($attr)
    {
...
if(!function_exists('surveyForUserAvailable'))
{
    function surveyForUserAvailable($survey_id, $user_id, $res_views)
    {
        if($res_views == 0) {
            return false;
        }
        $survey = Survey::whereId($survey_id)->first();
        if(!is_null($survey)) {
            if($survey->users) {
                if(in_array($user_id,$survey->users)){
                    return false;
                }
            }
...
if(!function_exists('surveyFavorite'))
{
    function surveyFavorite($survey_id, $user_id)
    {
        $count_fav = FavoriteSurvey::where('survey_id', $survey_id)->where('user_id', $user_id)->count();
        if($count_fav != 0) {
            return true;
        }
        return false;
...
if(!function_exists('surveyorProfileCompletePercent'))
{
    function surveyorProfileCompletePercent($surveyor)
    {
        $c_first_name = !is_null($surveyor->first_name) ? 2.5 : 0;
        $c_last_name = !is_null($surveyor->last_name) ? 2.5 : 0;
        $c_email = !is_null($surveyor->email) ? 5 : 0;
        $c_mobile = !is_null($surveyor->mobile) ? 5 : 0;
//        $c_balance = $surveyor->balance > 0 ? 10 : 0;
        $c_balance = 10;
        $c_image = !is_null($surveyor->image) ? 10 : 0;
        $c_address = $surveyor->address->address != '' ? 5 : 0;
        $c_city = $surveyor->address->city != '' ? 5 : 0;
        $c_state = $surveyor->address->state != '' ? 5 : 0;
        $c_zip = $surveyor->address->zip != '' ? 5 : 0;
        $c_business_cat = $surveyor->business_cat != '' ? 10 : 0;
//        $c_business_subcat = $surveyor->business_subcat != '' ? 10 : 0;
        $c_business_subcat = 10;
        $c_profiling_service = $surveyor->profiling_service != '' ? 10 : 0;
        $c_opt_out_msg = $surveyor->opt_out_msg != '' ? 5 : 0;
        if($surveyor->isfirm == 0) {
            $c_pan = $surveyor->pan != '' ? 15 : 0;
            $c_firm = $c_pan;
        }else{
            $c_firm_name = $surveyor->firm_name != '' ? 5 : 0;
            $c_firm_type = $surveyor->firm_type != '' ? 5 : 0;
            $c_designation = $surveyor->designation != '' ? 5 : 0;
            $c_firm = $c_firm_name + $c_firm_type + $c_designation;
        }
        return round($c_first_name + $c_last_name + $c_email + $c_mobile + $c_balance + $c_image + $c_address + $c_city + $c_state + $c_zip + $c_business_cat + $c_business_subcat + $c_profiling_service + $c_firm + $c_opt_out_msg);
    }
}
...
//        $file_name = time() . '_' . $new_file->getClientOriginalName();
        $file_name = uniqid() . time() . '.' . $new_file->getClientOriginalExtension();
...
if(!function_exists('prefernceList'))
{
    function prefernceList()
    {
        return $categories = Category::where('status','1')->get();
    }
}

if(!function_exists('isPreferance'))
{
    function isPreferance($preferance_name, $user_id)
    {
        $preferance = DB::table('preferences')->where('preferences_name', $preferance_name)->where('user_id', $user_id)->count();
        if($preferance == 0) {
            return false;
        }else{
            return true;
