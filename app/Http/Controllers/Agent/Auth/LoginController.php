...
class LoginController extends Controller
{
    ...
    */
    ...
    {
        $page_title = "Business Correspondent Login";
        return view('agent.auth.login', compact('page_title'));
    }
...
//
    ...
    {
        $page_title = 'Business Correspondent Recovery';
    ...
    public function authenticated(Request $request, $agent)
    {
        if ($agent->status == 0) {
            $this->guard('agent')->logout();
            return redirect()->route('agent.login')->withErrors(['Your account has been deactivated.']);
        }
