    ...
    public function authorizeForm()
    {
        ...
        if (auth()->check()) {
            ...
            if (!$user->status) {
                ...
                return view($this->activeTemplate.'user.auth.authorization.registration', compact('api_key','surveyor', 'page_title', 'reg_fee'));
            }else{
                return redirect()->route('user.home');
            }
    ...
    public function registrationPayment(Request $request)
    {
        ...
        if(count($input) && !empty($input['razorpay_payment_id'])) {
            try {
                ...
                $transaction->details = 'Payment For Registration Fee';
                $transaction->trx = $trx;
                $transaction->save();
